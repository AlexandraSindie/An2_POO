...
private:
	...
	int* locuri;
	char* denumireSala;
	const int intrari_iesiri;
	static string tipSala;//clasic,3d,5d
	int culoareTrecere;

public:
	Sala() : intrari_iesiri(2)
	{
		nrLocuri = 0;
		locuri = nullptr;
		denumireSala = new char[] {"Necunoscut"};
		culoareTrecere = 0;
	}

	Sala(const Sala& s) : intrari_iesiri(s.intrari_iesiri)
	{
		if (s.denumireSala != nullptr)
		{
			this->denumireSala = new char[strlen(s.denumireSala) + 1];
			strcpy_s(this->denumireSala, strlen(s.denumireSala) + 1, s.denumireSala);
		}
		else
		{
			this->denumireSala = nullptr;
		}
		...
		this->culoareTrecere = s.culoareTrecere;
	}
...
string Sala::tipSala = "Necunoscut";
